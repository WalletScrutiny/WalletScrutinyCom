...
containerApktool() {
  ...
  # Direct apktool call instead of using podman
  apktool d -o "$targetFolder" "$app"
  return $?
}

getSigner() {
  # Direct apksigner call instead of using podman
  apksigner verify --print-certs "$1" | grep "Signer #1 certificate SHA-256" | awk '{print $6}'
}
