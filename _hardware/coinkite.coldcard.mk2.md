...
⚠️ Warning (2026-08-02): Seeds generated on this device with firmware 4.0.1 through 4.1.9 have roughly 40 bits of entropy instead of the intended 128 and are considered predictable. Updating the firmware does not repair a seed that was already created — Coinkite advises updating to 4.2.0 or later, generating a new seed on the fixed firmware, and moving any funds to it. Seeds are not affected by this issue alone only if at least 50 fair, independent, private dice rolls were added when the seed was created, those rolls were never recorded or otherwise exposed, and the seed words used are the ones shown after adding them. See <a href="https://blog.coinkite.com/coldcard-mk3-seed-generation-warning/">Coinkite's security advisory</a> and the <a href="https://coldcard.com/docs/upgrade/">official firmware update instructions</a>.
</div> </div>
