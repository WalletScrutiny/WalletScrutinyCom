...
We have emailed the developer to confirm whether payments settle instantly to a destination the merchant controls or accrue in the Hydranode account until withdrawal, and whether a merchant can set their own payout destination. Should the answer show that funds never rest with a third party, this verdict would move to not a wallet; pending a reply we classify it `custodial` on the basis of the documented held-balance settlement model.
