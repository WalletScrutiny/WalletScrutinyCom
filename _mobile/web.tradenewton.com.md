---
wsId: newtonCryptoCanada
title: Newton - Trade Crypto Canada
verdict: custodial
meta: ok
date: 2026-01-05
authors:
- danny
website: https://newton.co
twitter: newton_crypto
social:
- https://discord.com/invite/newtoncrypto
- https://www.instagram.com/newton_crypto
- https://www.reddit.com/r/newtonco
- https://www.linkedin.com/company/newton-crypto
redirect_from:
- /android/web.tradenewton.com/
- /iphone/co.newton.newtonios/
android:
  appId: web.tradenewton.com
  users: 100000
  released: 2019-11-18
  updated: 2026-05-11
  version: 2.20.24
  reviews: 4
  icon: web.tradenewton.com.png
  developerName: Newton Crypto Ltd
iphone:
  appId: co.newton.newtonios
  idd: '1370504122'
  appCountry: ca
  released: 2018-10-03
  updated: 2026-05-12
  version: 2.20.24
  reviews: 24480
  icon: co.newton.newtonios.jpg
  developerName: Newton Crypto Ltd.

---

## Android

## App Description

This app is a Canadian-registered cryptocurrency trading platform operated by Newton Crypto Ltd., allowing users to buy, sell, and hold supported digital assets via a custodial account.

The app supports Bitcoin, along with dozens of other cryptocurrencies.

## Analysis

To quote from their FAQ: 

> we store the vast majority of our crypto in secure locations with no access to the internet. We also back up our keys in a distributed manner, meaning there are multiple redundancies in place to prevent accidental loss.
>
> Newton has partnered with Coinbase to provide institutional-grade storage for our customers' digital assets at no cost.

**This is a custodial service**.

---

## iPhone

{% include copyFromAndroid.html %}
