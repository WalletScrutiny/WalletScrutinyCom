---
wsId: bitsika
title: Bitsika - Crypto Virtual Card
altTitle: 
authors:
- danny 
users: 100000
appId: africa.bitsika.bitsika_mobile
appCountry: 
released: 2019-09-03
updated: 2025-12-14
version: 3.3.38
stars: 4.3
ratings: 
reviews: 117
website: https://www.bitsika.com
repository: 
issue: 
icon: africa.bitsika.bitsika_mobile.png
bugbounty: 
meta: ok
verdict: nowallet
appHashes: 
date: 2026-01-15
signer: 
twitter: bitsikaafrica
social:
- https://www.linkedin.com/company/bitsika
- https://www.instagram.com/bitsikaafrica 
redirect_from: 
developerName: Bit Sika
builds: 
features: 

---

## App Description

Bitsika’s Crypto Virtual Card is a digital Visa debit card you generate in the Bitsika app that you fund with Bitcoin or USDT and spend online or via Apple Pay/Google Pay wherever Visa is accepted.

Basically, it is a service that allows users to offramp to fiat through their virtual Visa debit card. It does not allow users to send out or withdraw their Bitcoins - only fiat.

## Analysis

Users deposited funds are held by BlockAfranew, Inc. on Bitsika’s platform in pooled accounts (not on user's own blockchain wallet), and can be transferred out to bank or mobile money accounts per the service’s transfer options rather than withdrawn back to an on-chain BTC address.** 

Despite advertising “No KYC” card creation and high top-up limits, [Bitsika’s terms](https://www.bitsika.com/terms-conditions) confirm that funds are custodially managed and subject to transfer controls, reviews, and limitations that are typical of centralized money transmission services.**

> You may transfer funds to any Mobile Money Account(s) or Banks Account(s) using the standard transfer option. We don’t offer the ability to access funds via ATM or at retail locations.

This is not a bitcoin wallet.