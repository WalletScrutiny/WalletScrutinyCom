class CustodianScore {
  ...
  logScore(category, item, points, reason) {
    ...
    if (typeof points === "number") {
      ...
      const sign = points >= 0 ? "+" : "";
      console.log(
        `[SCORE] ${category} > ${item}: ${sign}${points} points (${reason})`
      );
    } else {
      console.warn(
        `[SCORE WARNING] Invalid points value for ${category} > ${item}: ${points}`
      );
    }
  }
  ...
  calculateColdStorageScore() {
    ...
    // Add 5 points if status is partial
      ...
      this.logScore(
        ...
        "coldStorage",
        2,
        "Design is partially published"
      );
    }

    // Add 2 points if documentation URL exists and is not empty
    const hasDocUrl = hotCold.documentation_url && hotCold.documentation_url.trim() !== "";
    const hasSupportingUrls = hotCold.supporting_urls &&
      Array.isArray(hotCold.supporting_urls) &&
      hotCold.supporting_urls.some(url => url && url.trim() !== "");

    if (hasDocUrl || hasSupportingUrls) {
      score += 2;
      this.logScore(
        "keyManagement",
        "coldStorage",
  ...
  calculateProofOfReservesScore() {
    ...
    if (
      ...
      this.logScore(
        ...
        "proofOfReserves",
        5,
        "Proof of reserves or traditional audit supported"
      );
    }
    ...
    if (
      ...
      this.logScore(
        ...
        "proofOfReserves",
        1,
        "Recent proof or audit available"
      );
    }
  ...
  calculateMultiSigScore() {
    ...
    if (
      multiSig.supported ||
      (security.features &&
        security.features.includes("Multi-signature Wallets")) ||
      (hotCold.details &&
        (hotCold.details.includes("multi-signature") ||
      ...
      this.logScore(
        ...
        "multiSig",
        5,
        "Multi-signature/MPC/TSS is supported"
      );
    }

    // Check for implementation details (from multiple possible locations)
    if (
      (multiSig.details && multiSig.details.length > 0) ||
      (security.details &&
        security.details.includes("Multisignature Authorization")) ||
  ...
  calculateHardwareScore() {
    ...
    if (
      ...
      (security.details &&
        (security.details.includes("Hardware Wallets") ||
          security.details.includes("HSM"))) ||
      (hotCold.details &&
        (hotCold.details.includes("HSM") ||
      ...
      this.logScore(
        ...
        "hardware",
        5,
        "Hardware security is supported"
      );
    }

    // Check for specific hardware details (from multiple possible locations)
    if (
      (hardware.models && hardware.models.length > 0) ||
      (security.details &&
        (security.details.includes("Ledger") ||
      ...
      this.logScore(
        ...
        "hardware",
        5,
        "Specific hardware security details provided"
      );
    }
    ...
    if (certifications && certifications.length > 0) {
      ...
      if (typeof certifications[0] === "object") {
        ...
        if (validCertifications.length > 0) {
          ...
          score += certPoints;
          const certNames = validCertifications
            .map((cert) => cert.name)
            .join(", ");
          this.logScore(
            "infrastructure",
  ...
  calculateAuditsScore() {
    ...
    if (
      ...
      (securityAudits.auditReports &&
        securityAudits.auditReports.some(
          (report) => report && report.url && report.url.trim() !== ""
        ))
    ) {
      score += 5;
    ...
    if (
      securityAudits.insuranceCoverage &&
      securityAudits.insuranceCoverage.trim() !== ""
    ) {
      score += 2;
      this.logScore(
        "infrastructure",
...
// Section 2.3 Incident Response
  calculateIncidentResponseScore() {
    ...
    // Debug logging
    console.log('DEBUG: Incident Response - trackRecord:', JSON.stringify(trackRecord));

    // ❗ Require explicit signal that incidentHistory was reviewed
    if (
      !trackRecord.hasOwnProperty("incidentHistoryProvided") ||
      trackRecord.incidentHistoryProvided !== true
    ) {
      score = 0;
      console.log('DEBUG: Incident Response - incidentHistoryProvided flag not set');
      this.logScore(
        "infrastructure",
        "incidentResponse",
        0,
        "incidentHistoryProvided flag not set"
      );
      return { score, maxScore };
    }

    // 1) Manual override from front matter?
    if (typeof trackRecord.incidentSeverityScore === "number") {
      score = Math.max(-5, Math.min(trackRecord.incidentSeverityScore, 5));
      this.logScore(
        "infrastructure",
        "incidentResponse",
        score,
        "Manual override incidentSeverityScore"
      );
      return { score, maxScore };
    }

    // 2) Clean Track Record (+5)
    const incidents = trackRecord.incidentHistory || [];
    ...
    // 3) Multiple major incidents with no resolution (–5)
    if (incidents.some((i) => i.severity === "major" && !i.resolved)) {
      score = -5;
      this.logScore(
        "infrastructure",
        "incidentResponse",
        -5,
        "Multiple major incidents unresolved"
      );
      return { score, maxScore };
    }

    // 4) Minor incidents disclosed with resolution (+3)
    if (incidents.every((i) => i.severity === "minor" && i.resolved)) {
      score = 3;
      this.logScore(
        "infrastructure",
        "incidentResponse",
    ...
    // 5) Major incident(s), fully resolved (+1)
    if (
      incidents.every((i) => i.resolved) &&
      ...
      this.logScore(
        ...
        "incidentResponse",
        1,
        "Major incident(s) resolved"
      );
      return { score, maxScore };
    }

    // 6) Ongoing or unresolved incident (–2)
    console.log('DEBUG: Checking for unresolved incidents:', JSON.stringify(incidents));
    const hasUnresolvedIncident = incidents.some((i) => !i.resolved);
    console.log('DEBUG: Has unresolved incident:', hasUnresolvedIncident);
    
    if (hasUnresolvedIncident) {
      score = -2;
      console.log('DEBUG: Setting score to -2 for unresolved incident');
      this.logScore(
        "infrastructure",
        "incidentResponse",
        -2,
    ...
    // Fallback (shouldn't occur if data is correct)
    score = 0;
    this.logScore(
      "infrastructure",
      "incidentResponse",
      0,
      "Unexpected incident data"
    );
    return { score, maxScore };
  }
  ...
  calculateOpenSourceScore() {
    ...
    if (contrib.fossDevelopment) {
      ...
      this.logScore(
        ...
        "openSource",
        2,
        "Contributes to FOSS development"
      );
    }
    ...
    if (contrib.protocolSupport) {
      ...
      this.logScore(
        ...
        "openSource",
        2,
        "Supports Bitcoin protocol development"
      );
    }
  ...
  calculateLicensingScore() {
    ...
    if (leadership.jurisdiction) {
      ...
      this.logScore(
        ...
        "licensing",
        5,
        "Clear jurisdiction established"
      );
    }

    // Check for years in business (either directly or calculated from tenure)
    if (
      leadership.yearsInBusiness ||
      (leadership.ceo &&
        leadership.ceo.tenure &&
        leadership.ceo.tenure.includes("-"))
    ) {
      score += 3;
      this.logScore(
        "compliance",
    ...
    if (
      (leadership.team && leadership.team.length > 0) ||
  ...
  calculateComplianceProgramsScore() {
    ...
    if (model.type) {
      ...
      this.logScore(
        ...
        "compliancePrograms",
        2,
        "Business model supports compliance"
      );
    }

    if (
      model.restrictedCountries &&
      Array.isArray(model.restrictedCountries) &&
      model.restrictedCountries.length > 0
    ) {
      score += 3;
      this.logScore(
        "compliance",
        "compliancePrograms",
        3,
        "Complies with country restrictions"
      );
    }
  ...
  calculateAuthenticationScore() {
    ...
    if (
      security.biometric ||
      (security.features &&
        (security.features.includes("Biometric") ||
          security.features.includes("FIDO2")))
    ) {
      score += 2;
      this.logScore(
  ...
  calculateTransactionSecurityScore() {
    ...
    if (
      security.withdrawalConfirmations ||
      (security.features &&
        (security.features.includes("Withdrawal Confirmations") ||
          security.features.includes("2FA")))
    ) {
      score += 2;
      this.logScore(
        ...
        "transactionSecurity",
        2,
        "Withdrawal confirmations required"
      );
    }

    // Check for address whitelisting (from multiple possible locations)
    if (
      security.addressWhitelisting ||
      (security.features &&
        (security.features.includes("Withdrawal Whitelisting") ||
  ...
  calculateCategoryScore(category) {
    ...
    Object.entries(category.items).forEach(([itemId, item]) => {
      console.log(
        `  - Item ${itemId}: score=${item.score}, maxScore=${item.maxScore}`
      );
    });
    ...
    Object.entries(category.items).forEach(([itemId, item]) => {
      ...
      if (item && typeof item.score === "number") {
        ...
        console.warn(
          `Invalid score for ${categoryId}.${itemId}: ${item?.score}`
        );
      }
    });
    ...
    if (originalScore > category.maxScore) {
      this.logScore(
        categoryId,
        "categoryTotal",
        (originalScore - categoryScore) * -1,
        "Category score capped at maximum"
      );
    }
    ...
    console.log(
      `Final score for ${categoryId}: ${categoryScore}/${category.maxScore}`
    );
  ...
  calculateTotalScore() {
    ...
    if (!this.scoresCalculated) {
      ...
      const transactionSecurityResult =
        this.calculateTransactionSecurityScore();
      ...
      this.data.compliance.items.compliancePrograms.score =
        complianceProgramsResult.score;

      // Infrastructure
      this.data.infrastructure.items.certifications.score =
        certificationsResult.score;
      this.data.infrastructure.items.audits.score = auditsResult.score;
      this.data.infrastructure.items.incidentResponse.score =
        incidentResponseResult.score;

      // User Security
      this.data.userSecurity.items.authentication.score =
        authenticationResult.score;
      this.data.userSecurity.items.transactionSecurity.score =
        transactionSecurityResult.score;

      // STEP 3: Calculate category scores
      // Key Management
      const keyManagementScore =
        coldStorageResult.score + multiSigResult.score + hardwareResult.score;
      this.logScore(
        "keyManagement",
        "categoryTotal",
        coldStorageResult.score,
        "Added from coldStorage"
      );
      this.logScore(
        "keyManagement",
        "categoryTotal",
        multiSigResult.score,
        "Added from multiSig"
      );
      this.logScore(
        "keyManagement",
        "categoryTotal",
        hardwareResult.score,
        "Added from hardware"
      );
      ...
      if (keyManagementScore > this.data.keyManagement.maxScore) {
        this.logScore(
          "keyManagement",
          "categoryTotal",
          (keyManagementScore - this.data.keyManagement.maxScore) * -1,
          "Category score capped at maximum"
        );
      }
      this.data.keyManagement.score = Math.min(
        keyManagementScore,
        this.data.keyManagement.maxScore
      );
      ...
      const transparencyScore =
        proofOfReservesResult.score +
        documentationResult.score +
        openSourceResult.score;
      this.logScore(
        "transparency",
        "categoryTotal",
        proofOfReservesResult.score,
        "Added from proofOfReserves"
      );
      this.logScore(
        "transparency",
        "categoryTotal",
      ...
      if (transparencyScore > this.data.transparency.maxScore) {
        this.logScore(
          "transparency",
          "categoryTotal",
          (transparencyScore - this.data.transparency.maxScore) * -1,
          "Category score capped at maximum"
        );
      }
      this.data.transparency.score = Math.min(
        transparencyScore,
        this.data.transparency.maxScore
      );

      // Compliance
      const complianceScore =
        licensingResult.score + complianceProgramsResult.score;
      this.logScore(
        "compliance",
        "categoryTotal",
        licensingResult.score,
      ...
      if (complianceScore > this.data.compliance.maxScore) {
        this.logScore(
          "compliance",
          "categoryTotal",
          (complianceScore - this.data.compliance.maxScore) * -1,
          "Category score capped at maximum"
        );
      }
      this.data.compliance.score = Math.min(
        complianceScore,
        this.data.compliance.maxScore
      );
      ...
      if (infrastructureScore > this.data.infrastructure.maxScore) {
        this.logScore(
          "infrastructure",
          "categoryTotal",
          (infrastructureScore - this.data.infrastructure.maxScore) * -1,
          "Category score capped at maximum"
        );
      }
      this.data.infrastructure.score = Math.min(
        infrastructureScore,
        this.data.infrastructure.maxScore
      );

      // User Security
      const userSecurityScore =
        authenticationResult.score + transactionSecurityResult.score;
      this.logScore(
        "userSecurity",
        "categoryTotal",
        authenticationResult.score,
        "Added from authentication"
      );
      this.logScore(
        "userSecurity",
        "categoryTotal",
        transactionSecurityResult.score,
        "Added from transactionSecurity"
      );
      ...
      if (userSecurityScore > this.data.userSecurity.maxScore) {
        this.logScore(
          "userSecurity",
          "categoryTotal",
          (userSecurityScore - this.data.userSecurity.maxScore) * -1,
          "Category score capped at maximum"
        );
      }
      this.data.userSecurity.score = Math.min(
        userSecurityScore,
        this.data.userSecurity.maxScore
      );

      // STEP 4: Calculate total score
      const totalScore =
        this.data.keyManagement.score +
      ...
      this.logScore(
        ...
        "overall",
        this.data.keyManagement.score,
        "Added from keyManagement"
      );
      this.logScore(
        "totalScore",
        "overall",
        this.data.transparency.score,
        "Added from transparency"
      );
      this.logScore(
        "totalScore",
        "overall",
        this.data.compliance.score,
        "Added from compliance"
      );
      this.logScore(
        "totalScore",
        "overall",
        this.data.infrastructure.score,
        "Added from infrastructure"
      );
      this.logScore(
        "totalScore",
        "overall",
        this.data.userSecurity.score,
        "Added from userSecurity"
      );
  ...
  updateUI() {
    ...
    // Show Incident Response badge (colored by score)
    // Force recalculation of incident response score to ensure it's up-to-date
    const incidentResponseResult = this.calculateIncidentResponseScore();
    const incidentScore = incidentResponseResult.score; // Use the result directly
    
    console.log('DEBUG: Incident Response UI - Score from direct calculation:', incidentScore);
    
    // Store the score in the data structure
    this.data.infrastructure.items.incidentResponse.score = incidentScore;
    
    // Direct DOM manipulation - find the badge and value elements
    const badge = document.querySelector('.incident-response-badge');
    const value = document.querySelector('.incident-response-value');
    console.log('DEBUG: Incident Response UI - Badge element:', badge ? 'Found' : 'Not found');
    console.log('DEBUG: Incident Response UI - Value element:', value ? 'Found' : 'Not found');

    if (badge && value) {
      // Directly update the text content with the calculated score
      value.textContent = `${incidentScore}/5`;
      console.log('DEBUG: Incident Response UI - Updated text to:', `${incidentScore}/5`);
      ...
      } else if (incidentScore > 0) {
        badge.classList.add('incident-moderate');
      } else if (incidentScore === 0) {
        // Neutral styling for score of 0
        badge.classList.add('incident-moderate');
      } else {
        // Negative scores
    ...
    // Also update any other elements that might display the incident response score
    const allIncidentValues = document.querySelectorAll('.incident-response-value');
    allIncidentValues.forEach(el => {
      el.textContent = `${incidentScore}/5`;
    });
  ...
  addDebugPanel() {
    ...
    debugPanel.style.cssText =
      "position: fixed; bottom: 20px; right: 20px; width: 500px; max-height: 80vh; overflow-y: auto; background: rgba(0,0,0,0.8); color: #fff; padding: 15px; border-radius: 8px; font-family: monospace; z-index: 9999; font-size: 12px;";

    // Add header
    const header = document.createElement("h3");
    header.textContent = "Score Debug Panel";
    header.style.cssText =
      "margin-top: 0; color: #fff; border-bottom: 1px solid #555; padding-bottom: 8px;";
    ...
    closeBtn.style.cssText =
      "position: absolute; top: 10px; right: 10px; background: #f44336; color: white; border: none; border-radius: 50%; width: 24px; height: 24px; cursor: pointer;";
    ...
    categories.forEach((category) => {
      ...
      catDiv.style.cssText =
        "margin-bottom: 15px; padding: 10px; background: rgba(255,255,255,0.1); border-radius: 4px;";

      // Category header
      const catHeader = document.createElement("h4");
      catHeader.textContent = `${category.name}: ${cat.score}/${cat.maxScore}`;
      catHeader.style.cssText =
        "margin: 0 0 10px 0; color: #4CAF50; border-bottom: 1px solid #555; padding-bottom: 5px;";
      ...
      const rawScore = Object.values(cat.items).reduce(
        (sum, item) => sum + item.score,
        0
      );
      if (rawScore > cat.maxScore) {
        const rawScoreDiv = document.createElement("div");
        rawScoreDiv.style.cssText =
          "margin-top: 10px; font-style: italic; color: #ff9800;";
    ...
    Object.entries(groupedLogs).forEach(([category, entries]) => {
      ...
      categoryDiv.style.cssText =
        "margin-bottom: 15px; border-left: 3px solid #4CAF50; padding-left: 10px;";
      ...
      entries.forEach((entry) => {
        ...
        entryDiv.style.cssText =
          "margin: 5px 0; padding: 5px; background: rgba(255,255,255,0.1);";
        entryDiv.innerHTML = `<strong>${entry.item
          }</strong>: <span style="color: ${entry.points > 0 ? "#4CAF50" : "#f44336"
          };">${entry.points > 0 ? "+" : ""}${entry.points}</span> (${entry.reason
          })`;
