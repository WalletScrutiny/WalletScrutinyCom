...
function updateWalletGridInputOriginatingFromUI() {
  const platform = document.querySelector(".dropdown-platform .selected")
    ? document
        ...
        .getAttribute("data")
    : "allPlatforms";
  const page = document.querySelector(".pagination .selected")
    ? document.querySelector(".pagination .selected").innerHTML
    : 1;
  const queryRaw =
...
function buildWalletGridAndPaginationUI(platform, page, query, queryRaw) {
  ...
  console.log("Search results for", query, "on platform", platform, ":", workingArray);
...
function generateAndAppendWalletTiles(workingArray, pageNo) {
  ...
  for (let i = 0; i < paginationLimit; i++) {
    ...
    const domClass = String(`${wallet.folder}${String(wallet.appId)}`).replace(
      /\./g,
      "_"
    );
    ...
    let passed = ``;
    let failed = ``;
    for (let i = 0; i < wallet.score.numerator; i++) {
      passed += `<i class="pass"></i>`;
    }
    for (
      let i = 0;
      i < wallet.score.denominator - wallet.score.numerator;
      i++
    ) {
      failed += `<i class="fail"></i>`;
    }
    badgesHtml += `
    <a class="AppDisplayCard item ${wallet.folder} ${
      wallet.meta
    } ${domClass}" href="${wallet.url}" style="animation-delay:${delay}ms;">
      <div class="tile-head">
        <img src="${
          wallet.icon
            ? `/images/wIcons/${wallet.folder}/small/${wallet.icon}`
            : "/images/noimg.svg"
        }" class="app_logo" alt="Wallet Logo">
        <h3>${wallet.altTitle || wallet.title}</h3>
        <span class="platform tile-view-only"><i class="${icon}"></i><span> ${
      wallet.category
    }</span></span>
      </div>
      <div class="wallet-details">
        <div class="stamps">
        ${
          wallet.meta !== "outdated"
            ? `<span data-text="${
                window.verdicts[wallet.verdict].short
              }" class="stamp stamp-${wallet.verdict}" alt=""></span>`
            : ""
        }
        ${
          wallet.meta && wallet.meta !== "ok"
            ? `<span data-text="${
                window.verdicts[wallet.meta].short
              }" class="stamp stamp-${wallet.meta}" alt=""></span>`
            : ""
        }
        </div>
        <div class="score" data-numerator="${
          wallet.score.numerator
        }" data-denominator="${wallet.score.denominator}">
          <span>Passed ${
            wallet.score.numerator !== wallet.score.denominator
              ? wallet.score.numerator
              : "all"
          } ${
      wallet.score.numerator !== wallet.score.denominator ? "of" : ""
    } ${wallet.score.denominator} tests</span>
...
function generateAndAppendPagination(workingArray, pageNo) {
  if (
    workingArray.length >= paginationLimit &&
    Math.ceil(workingArray.length / paginationLimit) < pageNo
  ) {
    updateWalletGridInputOriginatingFromUI();
    return;
  }
  ...
  for (let i = 0; i < maxPages; i++) {
    ...
    clickTarget.addEventListener("click", (event) => {
      document
        .querySelector(".pagination .click-target.selected")
...
function generateDropdownAndInputCounts() {
  document.querySelector(".query-string").setAttribute(
    "placeholder",
    document
      ...
      .replace(
        /all/,
        String(window.wallets.length).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
      )
  );
}
...
function generateFeedbackText(
  ...
  const platformText = document
    .querySelector(`.dropdown-options.dropdown-platform .option.${platform}`)
    .getAttribute("data-name");
  if (queryRaw.length > 0 && workingArray.length === 0) {
    feedbackText = `No wallets match for <b>"${decodeURI(
      queryRaw
    )}"</b> in <i>${platformText}</i>.`;
  }
  
  else if (queryRaw.length > 0 && workingArray.length > 0) {
    feedbackText = `Found ${workingArray.length} wallets for <b>"${decodeURI(
      queryRaw
    )}"</b> in <i>${platformText}</i>.`;
}
...
function calcAllowedTargetArray(page, maxPages) {
  ...
  if (page < maxPages - 4) {
    ...
    end = [maxPages - 3, maxPages - 2, maxPages - 1];
  }
  middle = [page - 2, page - 1, page, page + 1, page + 2];

  for (const section of [beginning, middle, end]) {
    for (const number of section) {
      temp.push(number);
    }
  }
  temp = [...new Set(temp)];
  temp = temp.filter(function (numb) {
    return numb > -1;
  });
  for (let i = 0; i < temp.length; i++) {
    if (temp[i + 1] - temp[i] != 2) {
      allowedTargets.push(temp[i]);
    }
  }
...
async function processStyle(wallet) {
  ...
  imgObj.onload = function () {
    if (wallet.folder !== "bearer" && wallet.folder !== "hardware") {
      ...
      instanceCanvas.setAttribute(
        "class",
        `${domClass}-instance-canvas instance-canvas`
      );
      instanceCanvas.height = imgObj.height * 0.25;
      instanceCanvas.width = imgObj.width * 0.25;
      ...
      const canvasEleData = canvasEle
        .getContext("2d")
        .getImageData(0, 0, canvasEle.width, canvasEle.height);
      if (
        canvasEleData.data[3] == 0 &&
        canvasEleData.data[canvasEleData.data.length - 1] == 0
      ) {
        target.setAttribute("data-icon-shape", "free");
      } else if (canvasEleData.data[3] < 255) {
        target.setAttribute("data-icon-shape", "round");
      }
    }
    ...
    for (const rgb of colorThief.getPalette(imgObj, 3)) {
      ...
      if (target.style && target.style["background-image"]) {
        break;
      }
      target.style[
        "background-image"
      ] = `linear-gradient(var(--white) -80%, rgb(${rgb[0]},${rgb[1]},${rgb[2]}) 600%)`;
...
function setDropdown(parent, child) {
  if (document.querySelector(".dropdown-" + parent + " > ." + child)) {
    ...
    document
      .querySelector(".dropdown-" + parent + " > ." + child)
