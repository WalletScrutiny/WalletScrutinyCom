...
window.verdictCount = {};
let isInitializing = true; // Suppression flag for initial load
...
function updateWalletGridInputOriginatingFromUI() {
  ...
  window.history.pushState(
    'data',
    null,
    `/?platform=${platform}&page=${page}${queryRaw.length > 0 ? '&query-string=' : ''}${queryRaw}`
  );
...
function generateAndAppendWalletTiles(workingArray, pageNo) {
  ...
  for (let i = 0; i < paginationLimit; i++) {
    ...
    if (wallet.score) {
      ...
      for (let i = 0; i < (wallet.score.denominator - wallet.score.numerator); i++) {
...
function generateAndAppendPagination(workingArray, pageNo) {
  if (isInitializing) return; // Exit early during initialization
...
function setDropdown(parent, child) {
  if (document.querySelector(".dropdown-" + parent + " > ." + child)) {
    for (const ele of document.querySelectorAll(".dropdown-" + parent + " > .selected")) {
...
window.addEventListener("load", () => {
  updateWalletGridInputOriginatingFromURL();
  isInitializing = false; // Unset suppression flag after initial load
});
window.queryStringTimeout = false;
document.querySelector(".query-string").addEventListener("input", () => {
  if (isInitializing) return; // Skip if initializing
