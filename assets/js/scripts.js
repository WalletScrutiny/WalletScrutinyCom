...
/**
 * Count products per:
 * * platform-verdictGroup combination: window.verdictCount[platform][verdictGroup]
 * * platform: window.verdictCount[platform].all
 **/
...
function recreateDropdowns(verdict = 'allPassed', platform = 'all') {
  ...
  if (verdictGroups && document.querySelector(".dropdown-verdict")) {
    ...
    for (const [vgKey, vg] of Object.entries(verdictGroups)) {
      ...
      html += `<div class="option ${verdict === vgKey ? 'selected' : ''} ${vgKey}" data="${vgKey}" data-name="${vg.short}" data-value="${count}"><span>${vg.short}</span> <small>${count}</small></div>`
    }
...
function updateModularPayload(page, unrestrictedSearch, noPush) {
  ...
  document.querySelectorAll(".-filter-element").forEach(e => {
    e.classList.contains(`-${platform}`)
      ? (e.style.display = "flex")
      : (e.style.display = "none")
  })

  if (lastVerdict !== verdict || lastPlatform !== platform) {
  ...
  window.filteredWallets.forEach(wallet => {
    if (unrestrictedSearch) {
      presort.push(wallet)
    } else if (wallet.appId && wallet.verdict && wallet.folder &&
      verdictGroup.verdicts.includes(wallet.verdict) &&
      (verdictGroup.metas == undefined && wallet.meta === 'ok' ||
        verdictGroup.metas && verdictGroup.metas.includes(wallet.meta)
      ) &&
      (platform === "all" || String(wallet.folder) === platform)) {
      presort.push(wallet)
    }
  })
  if (!['hardware', 'bearer'].includes(platform) || unrestrictedSearch) {
    presort.sort((a, b) => {
    ...
    if (!persistRandomWallets) {
      presort.sort(() => Math.random() - 0.5) // TODO: this results in very poor randomization.
      persistRandomWallets = presort
    }
  }
  page = page ? page : 0
  const sortedWalletsForRender = persistRandomWallets
    ? persistRandomWallets
    : presort
  renderBadgesToDiv(sortedWalletsForRender, document.getElementById("modularWalletPayload"), page, verdict, platform)
  resizeLabelBold()
  if (!noPush) { updateUrl() }
  lastVerdict = lastVerdict !== verdict
    ? verdict
    : lastVerdict
  lastPlatform = lastPlatform !== platform
    ? platform
    : lastPlatform
}

const paginationLimit = 12
function renderBadgesToDiv(wallets, anchor, page, verdictGroup, platform) {
  page = page ? page : 0
  ...
  if (searchTerm.length === 0) {
    if (badgesHtml.length === 0) {
      let lessWorse = false
      for (let i = 1; i < document.querySelectorAll(".dropdown-verdict .option").length; i++) {
        const option = document.querySelectorAll(".dropdown-verdict .option")[i]
        if (option.getAttribute("data-value") > 0) {
          lessWorse = option
          break
        }
      }
      ...
      lessWorse = lessWorse
        ? `<br><a onclick="recreateDropdowns('${lessWorse.getAttribute("data")}', '${platform}');updateModularPayload(0, false)" class="primary btn">View highest-scoring ${platformTitleFormatting(platform)} wallets</a>`
        : ''
      if (platformNotes[platform]) {
        resultsText = `${platformNotes[platform]}<br>Learn more by exploring <a href="/methodology/?tests-we-run/${platform}/">our Methodology</a>.${lessWorse}`
      }
    }
    else {
      switch (verdictGroup) {
