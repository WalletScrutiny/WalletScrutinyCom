...
echo 'CREATE TABLE appStates (appId, title, date, users, latestUpdate, stars, ratings, reviews, size, verdict, revision);' >> /tmp/WS_SQL.SQL
...
do
  ...
  do
    ...
    appId="NULL";users="NULL";latestUpdate="NULL";stars="NULL";ratings="NULL";reviews="NULL";size="NULL";title="NULL";verdict="NULL"
    while read -r line
    do
      ...
      if [[ $line == "latestUpdate: "* ]]; then latestUpdate=$(date -d"${line#*: }" "+%s" --utc || continue); fi
    ...
    then
      ...
      sql="INSERT INTO appStates (appId, title, date, users, latestUpdate, stars, \
          ratings, reviews, size, verdict, revision) VALUES (\"$appId\", $title, \"$date\", $users, \
          \"$latestUpdate\", $stars, $ratings, $reviews, \"$size\", \"$verdict\", \
          \"$r\");"
