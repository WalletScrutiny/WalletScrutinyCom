...
class TrezorFirmwareExtractor {
    ...
    async getLegacyVersion() {
        try {
            this.log('Getting legacy version from GitHub tags...');
            const url = 'https://api.github.com/repos/trezor/trezor-firmware/tags?per_page=100';
            const response = await this.makeRequest(url);
            ...
            // Filter for legacy firmware tags (v1.x.x)
            const legacyTags = tags.filter(tag => 
                tag.name && tag.name.match(/^v1\.\d+\.\d+$/)
            );

            if (legacyTags.length === 0) {
                throw new Error('No legacy firmware tags found');
            }

            // Sort by version
            legacyTags.sort((a, b) => {
                const versionA = a.name.substring(1); // Remove 'v' prefix
                const versionB = b.name.substring(1);
                return this.versionCompare(versionB, versionA); // Descending order
            });

            const latestTag = legacyTags[0];
            
            // Get commit date
            let commitDate = new Date().toISOString().split('T')[0];
            try {
                const commitUrl = `https://api.github.com/repos/trezor/trezor-firmware/commits/${latestTag.commit.sha}`;
                const commitResponse = await this.makeRequest(commitUrl);
                const commit = commitResponse.json();
                if (commit && commit.commit && commit.commit.author) {
                    commitDate = commit.commit.author.date.split('T')[0];
                }
            } catch (error) {
                this.log(`Could not get commit date for ${latestTag.name}: ${error.message}`);
            }

            return {
                version: latestTag.name,
                date: commitDate,
                filename: `legacy/${latestTag.name}`,
                model_code: 'LEGACY',
