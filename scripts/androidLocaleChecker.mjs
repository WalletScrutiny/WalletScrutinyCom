...
const predefinedCountryCodes = 'US,CN,JP,IN,GB,CA,BR,AR,DE,ZA,AU,NZ,KR,FR,RU'.split(',');
 ...
 */
async function isAppAvailable(packageName, countryCode, retryCount = 0) {
  const url = `https://play.google.com/store/apps/details?id=${packageName}&gl=${countryCode}`;
  try {
    ...
    // Check if the app is not found or not available in the country
    ...
    // If we reach here, the app is likely available
    return true;
  } catch (error) {
    if (error.response && error.response.status === 429 && retryCount < maxRetries) {
      ...
      return isAppAvailable(packageName, countryCode, retryCount + 1);
    }
    // Don't log errors, just return false if there's an issue
    return false;
  }
}
...
async function writeLogFile() {
  ...
  await fs.writeFile('removedFixAndroid.log', logContent, 'utf8');
  if (totalFiles > 1) {
  ...
  console.log('Log file "removedFixAndroid.log" has been created.');
}

/**
 * Process a single file
 * @param {string} filePath - The path of the file to process
 * @param {string[]} codesToCheck - The list of country codes to check availability
 * @param {boolean} isSingleFile - Whether we are processing a single file
 * @returns {Promise<void>}
 */
async function processFile(filePath, codesToCheck, isSingleFile = false) {
  processedFiles++;

  let statusMessage = `Processing file ${processedFiles} of ${totalFiles}: ${path.basename(filePath)}`;
  ...
  try {
    ...
    if (content.header.meta === 'removed') {
      const packageName = content.header.appId;
      if (!packageName) {
        console.error(`Package name not found in content.header for ${path.basename(filePath)}`);
        return;
      }
      ...
      for (let i = 0; i < codesToCheck.length; i++) {
        ...
        const available = await isAppAvailable(packageName, code);
        if (available) {
      ...
      if (availableCountry) {
        statusMessage += ` - is available in ${availableCountry}`;
        ...
        helper.writeResult('_android/', content.header, content.body);
        updatedFiles.push(path.basename(filePath));
      } else {
        statusMessage += ' - is not available';
...
async function getFilesToProcess(filePathToProcess) {
  if (filePathToProcess) {
    ...
    const directoryPath = path.join(process.cwd(), '_android');
...
async function processFiles() {
  ...
  // If processing a single file, use all country codes, otherwise use predefined country codes
  const codesToCheck = filePathToProcess ? allCountryCodes : predefinedCountryCodes;

  // Print the header with country codes once
  const countryCodesTitle = codesToCheck.join(',');
  console.log(`\nChecking if the following apps are available in these countries: ${countryCodesTitle}`);
  console.log('-'.repeat(80) + '\n');

  const isSingleFile = totalFiles === 1;

  const promises = files.map((filePath) => limit(() => processFile(filePath, codesToCheck, isSingleFile)));
