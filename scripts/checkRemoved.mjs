...
const IOS_REGIONS = [
  'us', 'in', 'br', 'id', 'ru', 'mx', 'vn', 'tr', 'jp', 'gb', 
  'th', 'pk', 'ph', 'eg', 'de', 'fr', 'it', 'kr', 'ca', 'sa', 
  'ng', 'za', 'ke', 'au'
];
...
async function processAndroidApps() {
  try {
    ...
    try {
      ...
      console.log(`Warning: ${ANDROID_DIR} directory not found. Skipping Android apps.`);
      return;
    }
    ...
    for (const fileName of files) {
      ...
      if (header.meta === 'defunct') {
        ...
        process.stdout.write(`[${results.android.defunctChecked}] Checking defunct Android app: ${appId}... `);
        ...
        try {
          ...
          // App still exists
          console.log('✓ Still available in Play Store');
        } catch (error) {
          if (`${error}`.search(/404/) > -1) {
            // App is gone, update from defunct to removed
            header.meta = 'removed';
    ...
    for (const fileName of files) {
      ...
      if (header.meta === 'removed') {
        ...
        process.stdout.write(`[${results.android.removedChecked}] Checking removed Android app: ${appId}... `);
        ...
        try {
          ...
          console.log('✓ Found in Play Store - marking as ok');
        } catch (error) {
...
async function processIosApps() {
  try {
    ...
    try {
      ...
      console.log(`Warning: ${IOS_DIR} directory not found. Skipping iOS apps.`);
      return;
    }
    ...
    for (const fileName of files) {
      ...
      if (header.meta === 'defunct') {
        ...
        process.stdout.write(`[${results.ios.defunctChecked}] Checking defunct iOS app: ${appId}... `);
        ...
        const countriesToTry = [appCountry, ...IOS_REGIONS.filter(c => c !== appCountry)];
        let appFound = false;
        let lastError = null;
        ...
        if (!appFound) {
          if (`${lastError}`.search(/404/) > -1) {
            ...
            console.error(`⚠ Error checking app ${appId}: ${lastError.message}`);
    ...
    for (const fileName of files) {
      ...
      if (header.meta === 'removed') {
        ...
        process.stdout.write(`[${results.ios.removedChecked}] Checking removed iOS app: ${appId}... `);
        ...
        if (!idd) {
          ...
          try {
            ...
            if (lookupResult.length > 0) {
              ...
              console.log(`Found numeric App Store ID: ${numericId} for bundle ${appId}`);
              
              // App is back, update from removed to ok
              header.meta = 'ok';
              // Also update the idd field with the found numeric ID
              header.idd = numericId;
              helper.writeResult(IOS_DIR + '/', header, content.body);
              results.ios.removedToOk.push(appId);
              console.log('✓ Found in App Store - marking as ok');
            } else {
              console.log('✗ Still removed from App Store (bundle ID lookup failed)');
            }
          } catch (error) {
            console.log('✗ Still removed from App Store (bundle ID lookup error)');
          }
          continue;
        }
        ...
        for (const country of countriesToTry) {
          try {
            ...
            helper.writeResult(IOS_DIR + '/', header, content.body);
            results.ios.removedToOk.push(appId);
            console.log(`✓ Found in ${country} App Store - marking as ok`);
            break;
          } catch (error) {
            // Only continue to the next country if we got a 404
            if (`${error}`.search(/404/) === -1) {
              console.error(`⚠ Error checking app ${appId} in ${country}: ${error.message}`);
...
async function generateReports() {
  ...
  androidReport += formatSection('Apps Changed from Defunct to Removed', results.android.defunctToRemoved);
  androidReport += formatSection('Apps Changed from Removed to OK', results.android.removedToOk);
  androidReport += formatSection('Errors', results.android.errors.map(e => `${e.appId}: ${e.error}`));
  androidReport += `\n## Summary\n\n`;
  ...
  iosReport += formatSection('Apps Changed from Defunct to Removed', results.ios.defunctToRemoved);
  iosReport += formatSection('Apps Changed from Removed to OK', results.ios.removedToOk);
  iosReport += formatSection('Errors', results.ios.errors.map(e => `${e.appId}: ${e.error}`));
  iosReport += `\n## Summary\n\n`;
  ...
  combinedReport += `- Total defunct→removed changes: ${results.android.defunctToRemoved.length + results.ios.defunctToRemoved.length}\n`;
  combinedReport += `- Total removed→ok changes: ${results.android.removedToOk.length + results.ios.removedToOk.length}\n\n`;
