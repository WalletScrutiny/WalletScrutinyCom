async function getVersionInfo(octokit) {
  try {
    const githubData = (await octokit.rest.repos.getContent({owner: 'trezor', repo: 'data', path: 'firmware/1/releases.json'})).data
    const content = Buffer.from(githubData.content, 'base64').toString()
    const releases = JSON.parse(content)
    const commit = (await octokit.request('GET /repos/trezor/data/commits?path=firmware/1/releases.json&page=1&per_page=1')).data[0]
    return {
