...
4. companion: Tag software products (android/iphone/desktop) that act as the network-side companion to a hardware wallet or air-gapped signing device — i.e. the app that handles connectivity, transaction preparation, and broadcasting while the signing device holds the keys. Examples: Mycelium can connect to a Trezor via OTG cable (companion), Specter Desktop pairs with Specter-DIY (companion). NEVER tag hardware or bearer platform products with this feature — hardware wallets are the signing device, not the companion app. NEVER tag the signing device side of the pair (e.g. a software air-gapped signer). Tag the hot wallet side that talks to the hardware.
