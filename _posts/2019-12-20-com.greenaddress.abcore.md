...
- severity: medium
  ...
  message: >
    In the description on Google Play the provider writes:
    
    > ABCore is an Android app designed to make it easier to run a Bitcoin Core
      node on always on Android appliances such as set top boxes or repurposed
      tables/phones.
      
    > This app requires various gigabytes of disk space and is not to meant to
      run on 3G or 4G networks as it potentially downloads/uploads gigabytes of
      data.
    
    and on the [website](http://abco.re/en/releases/) we read:
    ...
    tons of resources, please investigate well if you want to use this as an
    actual wallet on your phone or maybe better only as a bitcoin full node on
    your Android TV.
- severity: low
  short: High resource usage!
  message: This app downloads the complete Bitcoin Blockchain.
date: 2020-03-02
reviewStale: false
signer: 
reviewArchive:
...
permalink: /posts/com.greenaddress.abcore/
redirect_from:
