...
import hardware from './scripts/helperHardware.mjs';
import bearer from './scripts/helperBearer.mjs';
import fs from 'fs';
...
async function refresh (markDefunct, apps, githubApi) {
  ...
  var msgAgeMs = 0;
  const i = setInterval(() => {
    const newMsg = `remaining: \
${playStore.stats.remaining + appStore.stats.remaining +
hardware.stats.remaining + bearer.stats.remaining}, \
🤖: defunct ${playStore.stats.removed}, updated ${playStore.stats.updated}, \
🍎: defunct ${appStore.stats.removed}, updated ${appStore.stats.updated}, \
🗃: defunct ${hardware.stats.defunct}, updated ${hardware.stats.updated}, \
💳: defunct ${bearer.stats.defunct}, updated ${bearer.stats.updated}`;
    ...
    if (playStore.stats.remaining + appStore.stats.remaining +
      hardware.stats.remaining + bearer.stats.remaining === 0 || msgAgeMs > 30000) {
      console.log('\nFinished.');
      clearInterval(i);
    }
  }, updateMillis);
