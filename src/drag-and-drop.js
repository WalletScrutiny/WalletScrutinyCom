...
async function displayAllInfo(dropAreaElement, file, apkInfo, hash, appData, allAssetsInformation) {
  ...
  if (window.location.hostname.includes('localhost') || window.location.hostname.includes('beta')) {  // Debug environment
    fileInfoHtml += `<strong>${await checkBlossomFile(hash, true) ? 'File exists in Blossom' : 'File does not exist in Blossom'}</strong> <small>(overrides cache - only shown in debug env)</small><br>`;
  }
